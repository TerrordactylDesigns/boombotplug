...
exports.LoadBlacklist = function(filepath) {
  try {
    ...
    filepath  = (filepath === undefined) ? './lib/models/store/blacklist.json' : filepath
    ...
    try {
      ...
      blacklist = {};
      fs.writeFileSync('./lib/models/store/blacklist.json', JSON.stringify(blacklist), 'ascii')
      console.log('*********Blacklist Loaded**********')
      return blacklist
    } catch (err) {
      console.log(err)
...
exports.LoadOptional = function(commands, filepath) {
  try {
    var filenames = fs.readdirSync('./lib/scripts')
    console.log('*****Loading Optional Scripts******')
    for (i = 0; i < filenames.length; i++) {
      if (filenames[i] != '.DS_Store') {
        console.log('Loading: ' + filenames[i])
        var command = require('../lib/scripts/' + filenames[i])
