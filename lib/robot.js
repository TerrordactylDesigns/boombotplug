...
exports.run = function(boombot, events, commands, config, version) {
  console.log('running')
  var Robot   = require('../lib/models/robot')
    , robot   = new Robot(boombot, events, commands, config, version)
    , connect = function() {boombot.connect(config.room)}
  /*<><><><><><><><><><><><><><><><><><><>
            API EMITTED EVENTS
  <><><><><><><><><><><><><><><><><><><>*/
  connect();
  ...
  boombot.on('curateUpdate', function(data){ events.curateUpdate(robot, data) })
  ...
  //boombot.on('close', connect())
  //boombot.on('error', connect())
}
